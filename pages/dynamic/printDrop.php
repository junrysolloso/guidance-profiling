...
<head>
  ...
  <?php
    ...
    `EmailAdd`, `Address`, `PhoneNo`, `LevelName`, `ProgramName`, `DropOutDate`
    ...
    INNER JOIN `drop_info` ON drop_info.DropNo = studentjunc_info.DropNo ");
  ?>
  <style>
    body {
    ...
    <table width="100%" class="table table-bordered table-hover" id="studentList">
      <thead>
        <tr>
          ...
          <th style="width:10%">Date Drop</th>
        </tr>
      </thead>
      <tbody>
        <?php
          if ($_SERVER['REQUEST_METHOD'] == 'GET') {
            if (isset($_GET['q'])) {
              ...
              $st = $conn->prepare(query." WHERE studentjunc_info.DropNo > 0 AND `ExpelledNo` = 0 AND `GraduateNo` = 0 AND `EducName` = ? ORDER BY `Lastname` ASC");
